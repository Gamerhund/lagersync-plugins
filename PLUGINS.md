...
| `verified` | bool | – | `true` = Plugin wurde getestet und als sicher verifiziert, `false` = Community-Plugin (nicht getestet) |
...
**Verified-Plugins wurden getestet und als sicher verifiziert:**
```json
...
# Plugin signieren
# (wird automatisch beim Build-Prozess für offizielle Plugins gemacht)
```
...
Plugins haben begrenzte API-Aufrufe pro Zeiteinheit:
...
def meine_daten():
    conn = get_db_connection()  # injiziert, kein Import nötig
    c = conn.cursor()
...
@require_auth()  # injiziert, kein Import nötig
def admin_bereich():
...
5. Nach Review und Test-Ausführung wird dein Plugin im Marketplace angezeigt
...
Plugins von unbekannten Quellen werden im Plugin-Manager mit **⚠️ Community** gekennzeichnet. Getestete und verifizierte Plugins tragen das **✅ Verifiziert** Badge (`"verified": true` in `plugin.json`).
