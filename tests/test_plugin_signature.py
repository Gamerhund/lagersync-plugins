    ...
    for plugin_path in plugin_dir.iterdir():
        if plugin_path.is_dir() and (not plugin_path.name.startswith('__')):
            ...
            if data.get('verified'):
                assert plugin_sig.exists(), f'Plugin {plugin_path.name}: Verifizierte Plugins müssen eine plugin.sig haben'

def test_signature_is_valid(plugin_dir, plugin_public_key):
    public_key = Ed25519PublicKey.from_public_bytes(base64.b64decode(plugin_public_key))
    for plugin_path in plugin_dir.iterdir():
        if plugin_path.is_dir() and (not plugin_path.name.startswith('__')):
            plugin_json = plugin_path / 'plugin.json'
            plugin_sig = plugin_path / 'plugin.sig'
