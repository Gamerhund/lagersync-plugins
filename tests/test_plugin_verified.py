...
def test_verified_plugins_author_gamerhund(plugin_dir):
    """Verifizierte Plugins müssen von Gamerhund sein"""
    for plugin_path in plugin_dir.iterdir():
        if plugin_path.is_dir() and not plugin_path.name.startswith("__"):
            plugin_json = plugin_path / "plugin.json"
            with open(plugin_json, 'r', encoding='utf-8') as f:
            ...
            if data.get("verified"):
                ...
                assert "Gamerhund" in author or "Jonas" in author, \
                    f"Plugin {plugin_path.name}: Verifizierte Plugins müssen von Gamerhund/Jonas sein"
